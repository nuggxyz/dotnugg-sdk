...
export class Renderer {
	...
	public static renderCheckCache(
		...
		for (var i = 0; i < builder.output.length; i++) {
			...
			cache[fileUri] = {
				mtimeMs,
				data: me.renderOnChain([builder.hexArray(builder.output[i])], true),
			};
			renderedamt++;
		...
		console.log(
			`rendered ${renderedamt} files and must "wait()" to save ${cachedamt} rendered files to cache`,
		);
	...
	public static async renderCheckCacheAsync(
		addr: string,
		prov: ethers.providers.InfuraProvider,
		dir: string,
		builder: dotnugg.builder,
		...
		console.log(
			`rendered ${renderedamt} files and calling "wait()" to save ${cachedamt} rendered files to cache`,
		);
